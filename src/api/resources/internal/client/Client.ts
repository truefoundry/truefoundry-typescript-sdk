...
export class InternalClient {
    ...
    private async __getIdFromFqn(type: string, request: TrueFoundry.InternalGetIdFromFqnRequest, requestOptions?: InternalClient.RequestOptions): Promise<core.WithRawResponse<Record<string, unknown>>> {
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), `api/svc/v1/fqn/${core.url.encodePathParam(type)}`),
            method: "GET",
            headers: _headers,
        ...
        if (_response.ok) {
            return { data: serializers.internal.getIdFromFqn.Response.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
