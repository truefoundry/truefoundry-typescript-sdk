...
export class ApplicationsClient {
    ...
    private async __promoteRollout(id: string, request: TrueFoundry.internal.ApplicationsPromoteRolloutRequest = {}, requestOptions?: ApplicationsClient.RequestOptions): Promise<core.WithRawResponse<void>> {
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), `api/svc/v1/x/apps/${core.url.encodePathParam(id)}/rollout/promote`),
            method: "POST",
            headers: _headers,
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "POST", "/api/svc/v1/x/apps/{id}/rollout/promote");
    }
    ...
    private async __getPodTemplateHashToDeploymentVersion(id: string, request: TrueFoundry.internal.ApplicationsGetPodTemplateHashToDeploymentVersionRequest = {}, requestOptions?: ApplicationsClient.RequestOptions): Promise<core.WithRawResponse<Record<string, number>>> {
        ...
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(_authRequest.headers, this._options?.headers, requestOptions?.headers);
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), `api/svc/v1/x/apps/${core.url.encodePathParam(id)}/pod-template-hash-deployment-version-map`),
            method: "GET",
            headers: _headers,
            queryString: core.url.queryBuilder().addMany(_queryParams).mergeAdditional(requestOptions?.queryParams).build(),
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
        ...
        if (_response.ok) {
            return { data: serializers.internal.applications.getPodTemplateHashToDeploymentVersion.Response.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/api/svc/v1/x/apps/{id}/pod-template-hash-deployment-version-map");
    }
}
