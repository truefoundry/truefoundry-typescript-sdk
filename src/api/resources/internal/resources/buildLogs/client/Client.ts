...
export class BuildLogsClient {
    ...
    private async __get(pipelineRunName: string, request: TrueFoundry.internal.BuildLogsGetRequest = {}, requestOptions?: BuildLogsClient.RequestOptions): Promise<core.WithRawResponse<TrueFoundry.LogsResponse>> {
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? await core.Supplier.get(this._options.environment), `api/svc/v1/x/build-logs/${core.url.encodePathParam(pipelineRunName)}`),
            method: "GET",
            headers: _headers,
        ...
        if (_response.ok) {
            return { data: serializers.LogsResponse.parseOrThrow(_response.body, { unrecognizedObjectKeys: "passthrough", allowUnrecognizedUnionMembers: true, allowUnrecognizedEnumValues: true, skipValidation: true, breadcrumbsPrefix: ["response"] }), rawResponse: _response.rawResponse };
        }
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/api/svc/v1/x/build-logs/{pipelineRunName}");
    }
}
