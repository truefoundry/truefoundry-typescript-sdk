    ...
    public async list(
        ...
        const list = core.HttpResponsePromise.interceptFunction(
            async (
                ...
                const { limit = 100, offset = 0, tag, fqn, promptId, mlRepoId, name, version } = request;
                ...
                if (_response.ok) {
                    return {
                        data: serializers.ListPromptVersionsResponse.parseOrThrow(_response.body, {
    ...
    private async __applyTags(
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            ...
            body: mergeAdditionalBodyParameters(
                serializers.ApplyPromptVersionTagsRequest.jsonOrThrow(request, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    omitUndefined: true,
    ...
    private async __get(
        ...
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)),
                `api/svc/v1/prompt-versions/${core.url.encodePathParam(id)}`,
            ),
            method: "GET",
            headers: _headers,
            queryString: core.url.queryBuilder().mergeAdditional(requestOptions?.queryParams).build(),
