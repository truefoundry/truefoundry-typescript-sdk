...
export class BearerAuthProvider implements core.AuthProvider {
    ...
    public static canCreate(options: BearerAuthProvider.Options): boolean {
        return options.apiKey != null || process.env?.TFY_API_KEY != null;
    }

    public async getAuthRequest(_arg?: { endpointMetadata?: core.EndpointMetadata }): Promise<core.AuthRequest> {
        const apiKey = (await core.Supplier.get(this.token)) ?? process.env?.TFY_API_KEY;
        if (apiKey == null) {
            throw new errors.TrueFoundryError({
                message:
                    "Please specify a apiKey by either passing it in to the constructor or initializing a TFY_API_KEY environment variable",
            });
        }
