// This file was auto-generated by Fern from our API Definition.

import type * as TrueFoundry from "../../api/index.js";
import * as core from "../../core/index.js";
import type * as serializers from "../index.js";

export const TrueFoundryAgentSandboxBasicAuthData: core.serialization.ObjectSchema<
    serializers.TrueFoundryAgentSandboxBasicAuthData.Raw,
    TrueFoundry.TrueFoundryAgentSandboxBasicAuthData
> = core.serialization.object({
    type: core.serialization.stringLiteral("basic"),
    username: core.serialization.string(),
    password: core.serialization.string(),
});

export declare namespace TrueFoundryAgentSandboxBasicAuthData {
    export interface Raw {
        type: "basic";
        username: string;
        password: string;
    }
}
